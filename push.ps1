...
if (-not $msg) {
    ...
    $existingNums = git log --oneline | ForEach-Object {
        if ($_ -match "V$today\.(\d+)") { [int]$Matches[1] }
    }
    $nextNum = if ($existingNums) { [int]($existingNums | Measure-Object -Maximum).Maximum + 1 } else { 1 }
    $numStr = "{0:D2}" -f $nextNum
    $versionPrefix = "V${today}.${numStr}"
    Write-Host "💡 推荐版本前缀：$versionPrefix" -ForegroundColor Yellow
    do { $desc = Read-Host "改动描述" } while (-not $desc)
    $msg = if ($desc -match "^V\d{8}") { $desc } else { "${versionPrefix}_${desc}" }
}
...
# 4. 同步到 Google Drive
$src = $PSScriptRoot
$dst = "O:\My Drive\050 - Script\01 - IM_SP_MasterData"

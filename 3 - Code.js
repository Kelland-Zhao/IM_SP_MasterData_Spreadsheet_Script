function updateSparePartsBasicInfo() {
  ...
  try {
    ...
    // 3. 读取备件基础信息现有数据，按同步规则更新 B/D/E，保留 C/F
    ...
    if (biLr > 1) {
      outputData = sbnBasicInfo.getRange(2, 1, biLr - 1, 6).getValues().map(row => {
        const mat       = String(row[0]).trim();
        ...
        if (mat && mdMap[mat]) {
          ...
          return [mat, mdMap[mat].desc, row[2], safeStock, mdMap[mat].stock, row[5]];
        } else {
          // 备件基础信息有但 MasterData 没有：更新 D，E 置 0，保留 B/C/F
          return [mat, row[1], row[2], safeStock, 0, row[5]];
        }
      });
    }
    ...
    // 5. 清空旧数据并一次性写入
    if (biLr > 1) sbnBasicInfo.getRange(2, 1, biLr - 1, 6).clearContent();
    if (outputData.length > 0) sbnBasicInfo.getRange(2, 1, outputData.length, 6).setValues(outputData);
